...
class Agent4:
    ...
    def a_star(self): #Main AStar Algorithm to calculate if a path exists or not
        ...
        while self.queue.__len__() > 0: #Traverse all the nodes until queue is not empty
            ...
            for i in range(0, 4):   #Traverse through the neighbours of the popped node
                temp_x = curr_x + Agent4.x_path[i] #Append the direction value to get the neighbours
                temp_y = curr_y + Agent4.y_path[i]
    ...
    def move_ghosts(self, ghost_x, ghost_y):    #Function to move a ghost in a random direction
        ...
        temp_ghost_x = ghost_x + Agent4.x_path[ghost_direction]
        temp_ghost_y = ghost_y + Agent4.y_path[ghost_direction]
    ...
    def move_agent(self, x, y): #Function to move the agent such that we don't land on a ghost cell
        for dir in range(4):
            temp_x = x + Agent4.x_path[dir]
            temp_y = y + Agent4.y_path[dir]
    ...
    def check_ghost_neighbourhood(self, x1, y1, ghost_location):    #Function to check if (x,y) is in the neighbourhood of any ghost
        for g in range(len(ghost_location)):
            ...
            for i in range(0, 5):   #Check in all five directions for each ghost
                if (ghost_x + Agent4.x_dir[i], ghost_y + Agent4.y_dir[i]) == (x1, y1):
                    return True
        return False

    def solve_agent4(self, path, ghosts):  #Function to traverse as Agent4 in the Grid for a given path and no. of ghosts
        ...
        while i < len(path):    #Traverse along the given path
            ...
            for g in range(ghosts): #After moving the agent, move all the ghosts based on given conditions
                ...
                self.discoveredGhosts[temp_ghost_x][temp_ghost_y] = 1  #Update the discovered grid for Agent4
            ...
            if i == len(path) - 1:  #If you reach at the last step of path, and it is not goal, reconstruct the path taking this as source
                ...
                path.extend(Agent4(startX, startY, self.goal_x, self.goal_y, self.size, self.matrix).solve_grid()[2][1:])

            for g in range(ghosts): #Check for all ghosts if the next step of the path if occupied by a ghost or its neighbourhood
                (x1, y1) = path[i + 1]
                if self.check_ghost_neighbourhood(x1, y1, ghost_location):   #If yes, Agent4 replans the path taking the current node as source node
                    (startX, startY) = path[i]
                    agent4_replan = Agent4(startX, startY, self.goal_x, self.goal_y, self.size, self.matrix, self.discoveredGhosts).solve_grid()
                    is_solvable = agent4_replan[0]
                    if is_solvable is False:  #If no solution possible at the moment, plan only the next step
                        ...
                        if next_step is False: #If no such step possible Agent4 dies
                            print('Agent4 is DEAD!')
                            return False, 0
                        ...
                        path[i + 1:] = agent4_replan[2]
            i += 1
        print('Agent4 SURVIVED!', len(path))   #If all the path is traversed, agent survives
        return True, len(path)


def plot_survivability_vs_ghosts_agent4(N, number_of_runs, ghost_upper_bound):  #Function to plot the graph of probability of survival vs ghosts for Agent4
    ...
    for iter in range(1, number_of_runs + 1):
        for i in range(5, ghost_upper_bound + 1, 5): #For each iteration, generate a matrix and check if agent survived for ghosts = 0 to 200
            ...
            if is_solvable[0] is True:   #Check if the matrix is solvable
                    survived[i] += Agent4(0, 0, N - 1, N - 1, N, matrix).solve_agent4(is_solvable[2], ghosts)[0]  #If yes, then traverse agent3 along the given solution path
    print(survived)
    ...
    plt.xticks(np.arange(0, ghost_upper_bound+1, 5))
    plt.title('AGENT 4 SURVIVABILITY', pad=30, fontweight="bold")
