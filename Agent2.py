...
class Agent2:
    ...
    def a_star(self): #Main AStar Algorithm to calculate if a path exists or not
        ...
        while self.queue.__len__() > 0: #Traverse all the nodes until queue is not empty
            ...
            for i in range(0, 4):   #Traverse through the neighbours of the popped node
                temp_x = curr_x + Agent2.x_path[i] #Append the direction value to get the neighbours
                temp_y = curr_y + Agent2.y_path[i]
    ...
    def move_ghosts(self, ghost_x, ghost_y):    #Function to move a ghost in a random direction
        ...
        temp_ghost_x = ghost_x + Agent2.x_path[ghost_direction]
        temp_ghost_y = ghost_y + Agent2.y_path[ghost_direction]
        return temp_ghost_x, temp_ghost_y

    def move_agent(self, x, y): #Function to move the agent2 such that we don't land on a ghost cell
        for dir in range(4):
            temp_x = x + Agent2.x_path[dir]
            temp_y = y + Agent2.y_path[dir]
    ...
    def solve_agent2(self, path, ghosts):   #Function to traverse as Agent2 in the Grid for a given path and no. of ghosts
        ...
        while i < len(path):    #Traverse along the given path
            ...
            if i == len(path) - 1:  #If you reach at the last step of path, and it is not goal, reconstruct the path taking this as source
                ...
                path.extend(Agent2(start_x, start_y, self.goal_x, self.goal_y, self.size, self.matrix).solve_grid()[2][1:])
            ...
            for g in range(ghosts): #Check for all ghosts if the next step of the path if occupied by a ghost
                if path[i + 1] == ghost_location[g]:
                    (start_x, start_y) = path[i]    #If yes, Agent2 replans the path taking the current node as source node
                    agent2_replan = Agent2(start_x, start_y, self.goal_x, self.goal_y, self.size, self.matrix, self.discoveredGhosts).solve_grid()
                    ...
                    if is_solvable is False:
                        ...
                        if next_step is False:  #If no such step possible Agent2 dies
                            print('Agent2 is DEAD!')
                            return False
            ...
            i += 1
        print('Agent2 SURVIVED!')   #If all the path is traversed, agent survives
        return True


def plot_survivability_vs_ghosts_agent2(N, number_of_runs, ghost_upper_bound):  #Function to plot the graph of probability of survival vs ghosts for Agent3
    ...
    for iter in range(1, number_of_runs + 1):
        ...
        is_solvable = Agent2(0, 0, N - 1, N - 1, N, matrix).solve_grid()
        print('solvable: ', is_solvable)    #Check if the matrix is solvable
        if is_solvable[0] is True:  #If yes, then traverse agent2 along the given solution path
            for i in range(ghost_upper_bound + 1):
                ghosts = i
                print('ghosts: ', ghosts)
                survived[i] += Agent2(0, 0, N - 1, N - 1, N, matrix).solve_agent2(is_solvable[2], ghosts)   #Count the number of times agent survived
    print(survived)
    ...
    plt.title('AGENT 2 SURVIVABILITY', pad=30, fontweight="bold")
